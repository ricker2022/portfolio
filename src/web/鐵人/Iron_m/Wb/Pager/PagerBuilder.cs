...
namespace Wb.Pager
{
    internal class PagerBuilder
    {
        ...
        private const string ScriptPageIndexName = "";
        ...
        {
            var item = new PagerItem(_pagerOptions.PrevPageText, _pageIndex - 1, _pageIndex == 1, PagerItemType.PrevPage, _pagerOptions.PrevPageParam);
        ...
        {
            var item = new PagerItem(_pagerOptions.FirstPageText, 1, _pageIndex == 1, PagerItemType.FirstPage, _pagerOptions.FirstPageParam);
            if (!item.Disabled || (item.Disabled && _pagerOptions.ShowDisabledPagerItems))
                results.Add(item);
        }
        ...
        {
            var item = new PagerItem(_pagerOptions.NextPageText, _pageIndex + 1, _pageIndex >= _totalPageCount, PagerItemType.NextPage, _pagerOptions.NextPageParam);
            //if (!item.Disabled || (item.Disabled && _pagerOptions.ShowDisabledPagerItems))
                results.Add(item);
        }
        ...
        {
            var item = new PagerItem(_pagerOptions.LastPageText, _totalPageCount, _pageIndex >= _totalPageCount, PagerItemType.LastPage, _pagerOptions.LastPageParam);
            if (!item.Disabled || (item.Disabled && _pagerOptions.ShowDisabledPagerItems))
                results.Add(item);
        }
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item)
        {
            ...
            if (item.Disabled) //first,last,next or previous page
                return CreateWrappedPagerElement(item, String.Format("<a {1} disabled=\"disabled\">{0}</a>", item.Text,item.Param));
            return CreateWrappedPagerElement(item,
                                             string.IsNullOrEmpty(url)
                                                 ? String.Format("<div>{0}</div>", item.Text)
                                                 : String.Format("<a {2} href='{0}'>{1}</a>", url, item.Text, item.Param));
        }
        ...
        private MvcHtmlString CreateWrappedPagerElement(PagerItem item, string el)
        {
            ...
            //if (item.Type == PagerItemType.FirstPage || item.Type == PagerItemType.PrevPage || item.Type == PagerItemType.NextPage || item.Type == PagerItemType.LastPage)
            if (item.Type == PagerItemType.PrevPage || item.Type == PagerItemType.NextPage || item.Type == PagerItemType.FirstPage || item.Text == _endPageIndex.ToString() || item.Type == PagerItemType.MorePage)
            {
                return MvcHtmlString.Create(navStr + _pagerOptions.DefaultSeparatorHtml);
            }
            else
            {
