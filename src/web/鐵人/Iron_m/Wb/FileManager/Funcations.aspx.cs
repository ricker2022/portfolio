    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       ...
       // AuthenticateFileManager();
        ...
        if (Request["funcation"] != null)
        {
            ...
            {
                if (string.IsNullOrEmpty(Request["Path"].ToString()) || Request["Path"].ToString() == "/" || Request["Path"].ToString() == "/upload/media/")
                {
   ...
   private void FilesULLI(string DirPath)
   {
       ...
       foreach (string File in Files)
       {
           ...
           if (
               ...
               && (
                       Request["type"].ToString() == "image"
                    || Request["type"].ToString() == "file"
                   )
              )
           {
               type = Request["type"].ToString();
               format = "," + ConfigurationManager.AppSettings["FileManager_Filter_" + type].ToString() + ",";
           }
           ...
           if(
                ...
                &&  (
                            ( format == "" )
                       ||   ( format.IndexOf( "," + f.Extension.Substring( 1 ) + "," ) >= 0  )
                    )
       ...
       DirectoryInfo d;
        foreach (string Dir in Directorys)
        {
            ...
            if (
                    ((d.Attributes & System.IO.FileAttributes.Hidden) != System.IO.FileAttributes.Hidden)
            )
            {
                //int FileType;
